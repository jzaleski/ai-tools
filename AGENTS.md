...
- Sampling defaults are tuned for coding and tool-calling per Qwen3.8's thinking/coding profile: `temp=0.6`, `top-k=20`, `top-p=0.95`, `min-p=0.0`, `presence-penalty=0.0`, `repeat-penalty=1.0`. Server-side `predict=32768` caps default output length (clients may override via `max_tokens`).
